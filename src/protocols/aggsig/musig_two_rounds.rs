...
impl KeyAgg {
    pub fn key_aggregation(my_pk: &GE, other_pk: &GE) -> KeyAgg {
        ...
        KeyAgg { apk, hash }
    //Doron: pk1,pk2 -> hash = H(1,pk1,pk1,pk2), H2 = (1,pk2,pk1,pk2), a1 = pk1*hash, a2 = pk2 *hash2,apk = a1+a2
    }
...
mod tests {
    ...
    fn test_multiparty_signing_for_two_parties() {
        ...
        assert!(EphemeralKey::test_com(
            ...
            party2_commitment
        )); //Doron: R,nonce,H(R,nonce) -> check equality
        ...
        // compute apk:
        let mut pks: Vec<GE> = Vec::new();//Doron: pks is [pk_1,...,pk_n]
