...
impl State {
    ...
    pub fn sign_2(&mut self, message: &[u8], pks:Vec<GE>, msg_vec: Vec<Vec<GE>>, party_index:usize) -> &State {
        ...
        let mut fixed_preimage: Vec<BigInt> = Vec::new();
        fixed_preimage.push(key_agg.apk.bytes_compressed_to_big_int());
        for i in 0..NUM_OF_SHARES {
            fixed_preimage.push(R_j_vec[i].bytes_compressed_to_big_int());
        }
        fixed_preimage.push(BigInt::from(message));

        for j in 1..NUM_OF_SHARES{
            let hnon_preimage: Vec<BigInt> = Vec::new();
            hnon_preimage: Vec<BigInt> = Vec::extend(fixed_preimage);
...
//            R = R.add_point(R_j_vec[j].scalar_mul(b_j));
    ...
    pub fn sign_3(msg2vec: Vec<FE>)-> FE{
        let s:FE = msg2vec.iter().
